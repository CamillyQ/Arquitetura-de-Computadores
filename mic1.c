...
/* =========================================================
   Execu��o de uma microinstru��o (23 bits)
   ========================================================= */
...
/* =========================================================
   Executor de IJVM (L�GICA FINAL E CORRETA DO PC)
   ========================================================= */
    ...
    while(read_line_trim(f,buf,MAX_LINE)){
        ...
        if(strncmp(buf,"ILOAD",5)==0){
            ...
            if ((uint32_t)(R.LV + x) > R.SP) {
                 ...
                 if (out2) {
                     fprintf(out2, "!!!! ERRO: ILOAD com endere�o acima do topo da pilha: LV(%u) + %d > SP(%u) !!!!\n", R.LV, x, R.SP);
                     fclose(out2);
                 }
                 // Em caso de erro, n�o executa e N�O avan�a o PC
            } else if ((uint32_t)(R.LV + x) >= MEM_LINES) {
                 FILE *out2 = fopen(LOG_FILE, "a");
                 if (out2) {
                     fprintf(out2, "!!!! ERRO: ILOAD com acesso a endere�o de mem�ria inv�lido !!!!\n");
                     fclose(out2);
                 }
                 // Em caso de erro, n�o executa e N�O avan�a o PC
            } else {
            ...
            R.PC += 1; // Sucesso: incrementa PC em 1 (para bater com a refer�ncia)
        }
    }
    fclose(f); return 1;
}
