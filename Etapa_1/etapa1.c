...
int load_initial_regs(const char *fname) {
    ...
    read_line_trim(f, buf, MAX_LINE); R.MBR = bin_to_u8(strchr(buf,'=')+1);
...
int generate_micro_ILOAD(int x){
    if ((uint32_t)(R.LV + x) > R.SP) {
        ...
        if (out) {
            fprintf(out, "========================================================\n");
            fprintf(out, "!!!! ERRO: ILOAD com endereco acima do topo da pilha: LV(%u) + %d > SP(%u) !!!!\n",
                    (unsigned)R.LV, x, (unsigned)R.SP);
            fprintf(out, "========================================================\n");
            fclose(out);
        }
        return 0; // indicate failure ? do not advance PC
    }
    ...
    execute_microinstr("00111000001000010100000"); // TOS = MDR
    return 1;
}

void generate_micro_DUP(){
    execute_microinstr("00110101000001001000100"); // MAR = SP=SP+1
    execute_microinstr("00010100000000010100111"); // MDR = TOS; wr
}
...
int run_ijvm_file(const char *fname){
    ...
    while(read_line_trim(f,buf,MAX_LINE)){
        ...
        if(strncmp(buf,"ILOAD",5)==0){
            int x=atoi(buf+6);
            int ok = generate_micro_ILOAD(x);
            if (ok) R.PC = R.PC + 1; // increment PC once per successful IJVM instruction
